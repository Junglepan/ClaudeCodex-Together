...
import { ChevronDown, ChevronRight, Pencil, Trash2, Check, X, AlertTriangle, Plus, Sparkles, Copy, Maximize2, Minimize2 } from 'lucide-react'
import { api } from '@/core/api'
import type { ApiSkillItem } from '@/core/api'
    ...
    return (
      <div className="flex items-center justify-center h-full">
        <div className="text-center space-y-2">
          ...
          <p className="text-sm font-medium text-text-primary">无法加载 Skills</p>
  ...
  return (
    <div className="h-full overflow-auto p-6 space-y-5 animate-fade-in">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-sm font-semibold text-text-primary">Skills 管理</h2>
          <p className="text-2xs text-text-tertiary mt-0.5">
            {agentId === 'claude'
              ? '自定义斜杠命令，存放于 .claude/skills/<name>/SKILL.md'
              : '自定义技能文件，存放于 .codex/skills/<name>.md'}
          </p>
        </div>
      ...
      {skills.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-16 text-center">
          ...
          <p className="text-sm font-medium text-text-primary">暂无 Skills</p>
          <p className="text-xs text-text-tertiary mt-1 max-w-xs">
            {agentId === 'claude'
              ? '在 ~/.claude/skills/ 或 .claude/skills/ 目录下创建 SKILL.md 来添加自定义斜杠命令'
              : '在 ~/.codex/skills/ 或 .codex/skills/ 目录下创建 .md 文件来添加自定义技能'}
          </p>
        </div>
      ) : (
        <>
          {globalSkills.length > 0 && (
            <SkillGroup label="全局 Skills" skills={globalSkills} onRefresh={load} />
          )}
          {projectSkills.length > 0 && (
            <SkillGroup label="项目 Skills" skills={projectSkills} onRefresh={load} />
          )}
        </>
      )}
    </div>
...
function SkillGroup({ label, skills, onRefresh }: { label: string; skills: ApiSkillItem[]; onRefresh: () => void }) {
  return (
    <div className="bg-surface-card border border-border-default rounded-xl overflow-hidden">
      <div className="px-4 py-3 border-b border-border-subtle flex items-center gap-2">
        ...
        <span className="text-2xs text-text-tertiary">{skills.length} 个</span>
      </div>
      <div className="divide-y divide-border-subtle">
        {skills.map((skill) => (
          <SkillCard key={skill.path} skill={skill} onRefresh={onRefresh} />
        ))}
      </div>
    </div>
  )
...
function SkillCard({ skill, onRefresh }: { skill: ApiSkillItem; onRefresh: () => void }) {
  ...
  const truncated = !contentExpanded && skill.content.length > MAX_PREVIEW
  ...
  const handleSave = async () => {
    setSaving(true)
    try {
      await api.files.write(skill.path, editContent)
      pushToast({ kind: 'success', message: `已保存：${skill.name}` })
      setEditing(false)
      onRefresh()
    } catch (e) {
  ...
  const handleDelete = async () => {
    setDeleting(true)
    try {
      await api.files.delete(skill.path)
      pushToast({ kind: 'success', message: `已删除：${skill.name}` })
    ...
    <div className="group">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            ...
            <span className={`text-2xs font-medium px-1.5 py-0.5 rounded ${
              skill.source === 'global' ? 'bg-accent-blue/10 text-accent-blue' : 'bg-accent-green/10 text-accent-green'
            }`}>
              {skill.source === 'global' ? '全局' : '项目'}
            </span>
          </div>
          {skill.description && (
            <p className="text-2xs text-text-tertiary mt-0.5 truncate">{skill.description}</p>
          )}
        </div>
      ...
      {confirmDelete && (
        <div className="mx-4 mb-3 p-3 bg-red-50 border border-red-200 rounded-xl flex items-start gap-3 animate-fade-in">
          ...
          <div className="flex-1">
            <p className="text-xs font-medium text-red-800">确认删除 "{skill.name}"？</p>
            <p className="text-2xs text-red-600 mt-0.5 font-mono break-all">{skill.path}</p>
          </div>
      ...
      {expanded && (
        <div className="px-4 pb-4 animate-fade-in">
          <div className="text-2xs text-text-tertiary font-mono mb-2 truncate">{skill.path}</div>
            ...
            <div className="border border-border-default rounded-lg overflow-hidden">
              <div className="px-3 py-1.5 bg-surface-base border-b border-border-subtle flex items-center justify-between">
                <span className="text-2xs text-text-tertiary">
                  {skill.content.split('\n').length} 行
                  {truncated ? `（共 ${skill.content.split('\n').length} 行）` : ''}
                </span>
                <div className="flex items-center gap-2">
                  {skill.content.length > MAX_PREVIEW && (
                    <button
              ...
              <pre className={`p-3 text-xs font-mono text-text-primary overflow-auto leading-relaxed bg-surface-card ${contentExpanded ? 'max-h-[70vh]' : 'max-h-48'}`}>
                {truncated ? skill.content.slice(0, MAX_PREVIEW) : skill.content}
