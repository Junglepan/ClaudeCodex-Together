...
export function Sidebar() {
  ...
  // Partition: bottom modules vs main modules
  ...
  // Group main modules: ungrouped items first, then grouped sections
  const ungrouped = mainModules.filter((m) => !m.group)

  // Collect unique group names in registration order
  const groupNames = Array.from(
    new Set(mainModules.filter((m) => m.group).map((m) => m.group!))
  )

  return (
    <aside className="w-[164px] flex-shrink-0 bg-surface-base border-r border-border-default flex flex-col select-none">
      {/* Main nav */}
      ...
      <div className="px-3 pb-4 space-y-2.5">
        {/* Local-only badge */}
        <div className="flex items-start gap-1.5 p-2.5 bg-accent-blue/5 border border-accent-blue/15 rounded-lg">
...
function NavItem({ mod }: { mod: ReturnType<typeof moduleRegistry.getNavItems>[number] }) {
  return (
    <NavLink
      to={mod.path}
      className={({ isActive }) =>
