...
浏览和回顾 Claude Code / Codex CLI 的所有历史对话，以三栏布局呈现：

**左侧边栏 — 项目树 + 会话列表**
- 自动发现 `~/.claude/projects/` 和 `~/.codex/sessions/` 下的所有会话文件
- 按项目分组展示，支持折叠展开，显示会话数量和最后活跃时间
- 全局搜索会话内容，支持角色和工具名过滤

**中间主区域 — 概览仪表盘 / 对话查看器**
- **概览模式**：统计卡片（总会话数 / 总消息数 / 总大小 / 近 7 天活跃）、Agent 分布、工具使用 TOP 8、技能使用 TOP 8、项目卡片网格
- **对话模式**：聊天气泡风格展示（用户消息蓝色右对齐 / AI 回复白色左对齐 / 工具调用蓝色边框 / 工具结果琥珀色边框 / 错误红色高亮）
  - 角色过滤（全部 / 用户 / AI）、工具消息开关
  - 分页加载（默认最后 50 条，支持前后翻页）
  - 长工具输出自动折叠，点击展开
  - 一键复制：项目路径、文件路径、Session ID、Resume 命令（`claude --resume <uuid>`）

**右侧面板 — 统计 + 导航**
- 会话统计：消息角色分布、工具调用次数与失败率、使用的工具/技能/子代理列表
- 消息导航：按角色快速跳转到对应消息

**Codex 会话支持**：独立解析器完整支持 Codex JSONL 格式（session_meta / event_msg / response_item），正确提取项目路径、会话标题和工具调用。
...
| `⌘R` | 刷新所有数据 |
| `⌘B` | 切换侧栏折叠 |
| `⌘1` ~ `⌘5` | 切换模块（概览 / 配置管理 / 同步中心 / Claude 会话 / Codex 会话） |
| `/` | 聚焦搜索框 |
| `?` | 打开快捷键速查表 |
...
│       └── sessions/       #   会话解析（Claude / Codex）、搜索、统计、回收
